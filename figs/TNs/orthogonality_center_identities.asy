...
picture T_D_pic;
currentpicture = T_D_pic;
TensorNetwork net_T_D = makeTensorNetwork(T_D);
draw(net_T_D);

label("$\dots$", (dist*len_center,0), fontsize(20));
label("$\dots$", (dist*len_center,bra_off_up), fontsize(20));
// arc cricle
path arc = arc((dist*(len_center +0.5 ),bra_off_up/2), bra_off_up/2, 90, -90);
draw(arc, linewidth(2));

label("$=$", (dist*(len_center + 2),bra_off_up/2), fontsize(20));
draw(shift((dist*(2),0))*arc, linewidth(2));
...
real total_offset = 0;
real dots_length = 50;
picture[] pics = new picture[] {T_G_pic};
...
for (int i = 0; i < pics.length; i+=1) {
  ...
  if (i < pics.length - 1) {
    label("$,$", (total_offset - dots_length/2, bra_off_up/2));
  }
}
