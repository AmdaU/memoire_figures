...
Leg i_1 = makeLeg("i_1", orientation, dim=i_dim);
Leg i_2 = makeLeg("i_2", orientation, dim=i_dim);
Leg i_3 = makeLeg("i_3", orientation, dim=i_dim);
Leg i_4 = makeLeg("i_4", orientation, dim=i_dim);
...
Tensor CD_0 = makeTensor("${\rm CD}(s)$", rotate(rot_angle)*(0, total_y), new Leg[] {dag(q_0), dag(i_0), q_1, i_1}, secondary, "rect", ratio=rect_ratio, r=rect_r);
total_y += v_dist;

Tensor R_0 = makeTensor("${\rm R}_x(\frac{\pi}{2})$", rotate(rot_angle)*(-r, total_y), new Leg[] {dag(q_1), q_2}, tertiary, "rect", r=r, ratio=0.9);
Tensor K_i = makeTensor("${\rm K}_i$", rotate(rot_angle)*(r, total_y), new Leg[] {dag(i_1), i_2}, primary, "rect", r=r, ratio=0.9);
total_y += v_dist;

Tensor CD_1 = makeTensor("${\rm CD}(B)$", rotate(rot_angle)*(0, total_y), new Leg[] {dag(q_2), dag(i_2), q_3, i_3}, secondary, "rect", ratio=rect_ratio, r=rect_r); 
total_y += v_dist;

Tensor R_1 = makeTensor("${\rm R}_x^\dagger(\frac{\pi}{2})$", rotate(rot_angle)*(-r, total_y), new Leg[] {dag(q_3), q_4}, tertiary, "rect", r=r, ratio=0.9);
Tensor K_j = makeTensor("${\rm K}_j$", rotate(rot_angle)*(r, total_y), new Leg[] {dag(i_3), i_4}, primary, "rect", r=r, ratio=0.9);
total_y += v_dist;
...
TensorNetwork net_full = makeTensorNetwork(new Tensor[] {ket, CD_0, R_0, K_i, CD_1, R_1, K_j, CD_2, bra});
...
TensorNetwork net_contracted_ket_and_bra = makeTensorNetwork(new Tensor[] {CD_0_p, R_0, K_i, CD_1, R_1, K_j, CD_2_p});
...
Tensor sBs_m = makeTensor("${\rm sBs}^m_{i,j}$", rotate(rot_angle)*(0, total_y), new Leg[] {dag(i_0), i_5}, secondary, "square", r=r);
