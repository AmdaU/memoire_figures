...
for (int n = 0; n < len_ends; n+=1) {
  legs_physical_start[n] = makeLeg("i_{" + string(n+1) + "}", (0, 1), dim=dim_physical);
  int dim = dim_edges+n;
...
for (int n = 0; n < len_ends; n+=1) {
  ...
  T_start[n] = makeTensor("", (n*dist,0), legs_start, primary, "circle");
  T_end[n] = makeTensor("", ((n)*dist,0), legs_end, primary, "circle");
}
...
for (int n = start_index; n <= end_index; n+=1) {
  ...
  T_center[n+offset] = makeTensor("", (n*dist,0), legs_center, primary, "circle");
}
