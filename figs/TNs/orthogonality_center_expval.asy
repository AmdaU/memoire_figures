...
label("$\dots$", (dist*(len_center),bra_off_up), fontsize(20));
...
Tensor T_G_contracted = contract("", makeTensorNetwork(T_G), shape="id_v", pos=(dist*(ortho_index-1),bra_off_up/2));
Tensor T_D_contracted = contract("", makeTensorNetwork(T_D), shape="id_v", pos=(dist*(ortho_index+1),bra_off_up/2));
...
real total_offset = 0;
real dots_length = 35;
picture[] pics = new picture[] {pic_ket_bra, contracted_pic};

for (int i = 0; i < pics.length; i+=1) {
  total_offset += -min(pics[i]).x;
  attach(pics[i].fit(), (total_offset,0));
  total_offset += max(pics[i]).x + dots_length/2;
  if (i < pics.length - 1) {
    label("$=$", (total_offset, bra_off_up/2), fontsize(20));
  }
  total_offset += dots_length;
}
