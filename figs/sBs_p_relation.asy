...
for (int i = 0; i < n; ++i) {
	...
	label("\scalebox{3}{$\ket{" + string(logical_state) + "," + string(i) + ",0}$}", (x_pos, pdf_label_offset), NoAlign, textcolor);
}
...
for (int i = 0; i < n - 1; ++i) {
	...
	draw((x_start, y_center + arrow_gap) -- (x_end, y_center + arrow_gap), p=arrow_pen, 
	     ...
	     L=Label("$K_{\text{sBs}, p}^\dagger$", position=MidPoint, align=N, fontsize(label_fontsize*arrow_scale) + textcolor));
	
	// Arrow pointing left (below)
	draw((x_end, y_center - arrow_gap) -- (x_start, y_center - arrow_gap), p=arrow_pen, 
	     arrow=Arrow(TeXHead, size=arrow_size*arrow_scale), 
	     L=Label("$K_{\text{sBs}, p}$", position=MidPoint, align=S, fontsize(label_fontsize*arrow_scale) + textcolor));
}
