...
path boundary = box(((grid_l-2)*l, (grid_l+2)*l/2), (-(grid_l+10)*l,-(grid_l+10)*l/2)); // the boundary

real rad = (grid_l+1) * l/sqrt(2);

real envelope_keep(pair p) {
  real r = length(p)/rad;
  return exp(-delta_like/rad*r*r); // same profile as previous overlay
}
...
label(label_string, (-line_height, i*l), fontsize(fontsize) + textcolor, align=E);
}


yaxis(ymin=-((n+1/2)+0.75)*l, ymax=((n+1/2)+0.75)*l, p=axis_pen,  Arrows(5));
labely("$\hat p$",2*W, fontsize(fontsize*2) + textcolor);
xaxis(Label("$|\psi(p)|^2$", fontsize(fontsize*1.2) + textcolor, align=N), YEquals((grid_l+1/2)*l/2), axis_pen, EndArrow(5), xmin=0, xmax=line_height*1.2);
